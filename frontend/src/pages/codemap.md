...
| `NoteDetail` | `/notes/:id` | Full note view: transcript, AI image, audio playback, similar notes |
| `Gallery` | `/gallery` | Grid of AI-generated note images with date labels |
...
3. `processNote()` called → transcribe → generate image → generate embedding (sequential async).
